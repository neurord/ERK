...
slopenorm_mean=df.groupby('filename').mean()['slope_norm']
stdnorm=df.groupby('filename').std()['slope_norm']
plt.figure()
...
jg = sns.jointplot(df['bestITI'],df['slope_norm'],ratio=3,xlim=(0,df['bestITI'].max()*1.1),ylim=(0,df['slope_norm'].max()*1.1),hue=filename)#,s=10,alpha=.8,edgecolor='0.2',linewidth=.2,color='k')
...
Yslope = df['slope_norm']
Yiti = df['bestITI']

for y,label in zip([Yslope,Yiti],['slope_norm','bestITI']):
...
num_plots=2
mols=[feature_importance_df.index[int(feature_importance_df.iloc[i])] for i in range(num_plots)]
plt.figure()
plt.title('idk')#IDK WHAT THIS IS DOING, SEEMS THAT IT IS PREDICTING 
for mol in mols:
...
for i,ii in sortedpval:
     statsmol.append(ii)   
mod = sm.OLS(y_train,X_train[statsmol])#['RasGRF','Epac','MKP1']])
res = mod.fit()
res.summary()
