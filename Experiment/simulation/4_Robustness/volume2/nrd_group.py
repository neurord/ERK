...
class nrdh5_group(object): 
    ...
    def trace_features(self,trials,window_size,lo_thresh_factor=0.2,hi_thresh_factor=0.8,std_factor=1,numstim=1):
        import operator
        self.feature_list=['baseline','basestd','peakval','peaktime','amplitude','duration','slope','minval','auc','auc_thresh','start_plateau']
        self.feature_dict={feat:np.zeros((len(self.ftuples),len(self.molecules),len(trials))) for feat in self.feature_list}
        self.mean_feature={feat:np.zeros((len(self.ftuples),len(self.molecules))) for feat in self.feature_list}
        self.std_feature={feat:np.zeros((len(self.ftuples),len(self.molecules))) for feat in self.feature_list}
        ...
        for parnum,(fname,par) in enumerate(self.ftuples):
            for imol,mol in enumerate(self.molecules):
                ...
                self.feature_dict['baseline'][parnum,imol,:]= np.mean(self.file_set_conc[par][mol][:,self.sstart[mol]:self.ssend[mol]],axis=1)
                self.feature_dict['basestd'][parnum,imol,:]=np.std(self.file_set_conc[par][mol][:,self.sstart[mol]:self.ssend[mol]],axis=1)
                peakpt=np.argmax(self.file_set_conc[par][mol][:,self.ssend[mol]:],axis=1)+self.ssend[mol]
                self.feature_dict['peaktime'][parnum,imol,:]=peakpt*self.dt[mol]
                self.feature_dict['peakval'][parnum,imol,:]=[np.mean(self.file_set_conc[par][mol][i,p-window:p+window]) 
                                                             for i,p in enumerate(peakpt)]
                lowpt=np.argmin(self.file_set_conc[par][mol][:,self.ssend[mol]:],axis=1)+self.ssend[mol]
                self.feature_dict['minval'][parnum,imol,:]=[np.mean(self.file_set_conc[par][mol][:,p-window:p+window]) 
                                                            for i,p in enumerate(lowpt)]
                self.feature_dict['amplitude'][parnum,imol,:]=self.feature_dict['peakval'][parnum,imol,:]-self.feature_dict['baseline'][parnum,imol,:]
                ####################
                #FIND SLOPE OF INCREASE using - Use thresholds defined by lo_thresh, and hi_thresh, e.g. 20 and 80%
                lo_thresh=lo_thresh_factor*(self.feature_dict['amplitude'][parnum,imol,:])+self.feature_dict['baseline'][parnum,imol,:] #get the 5% above the max value
                hi_thresh=hi_thresh_factor*(self.feature_dict['amplitude'][parnum,imol,:])+self.feature_dict['baseline'][parnum,imol,:]
                ...
                for i,(end,beg) in enumerate(zip(end_slope,begin_slope)):
                    if end-beg>1 and ~np.isnan(end) and ~np.isnan(beg): #FIX - check for end=beg - making zero slope, or even end_slope.beginslope=1
                        self.feature_dict['slope'][parnum,imol,i]=(self.feature_dict['peakval'][parnum,imol,i]-
                                                                   self.feature_dict['baseline'][parnum,imol,i])/((end-beg)*self.dt[mol])
                    else:
                        self.feature_dict['slope'][parnum,imol,:]=np.nan                    
                ####################
                ...
                midpoints=0.5*(self.feature_dict['amplitude'][parnum,imol,:])+self.feature_dict['baseline'][parnum,imol,:]
                ...
                self.feature_dict['start_plateau'][parnum,imol,:]=[platpt*self.dt[mol] for platpt in start_platpt]
                #print('plateau',self.feature_dict['start_plateau'][parnum,imol,:],end_platpt)
                self.feature_dict['duration'][parnum,imol,:]=[(end-start)*self.dt[mol] 
                ...
                if numstim>1 and len(par):
                    stim_time=int((float(par[-1])*numstim)/self.dt[mol]+self.ssend[mol])#get previous to last stimuation time
                else:
                    stim_time=self.ssend[mol] 
                self.feature_dict['auc_thresh'][parnum,imol,:]=(self.feature_dict['baseline'][parnum,imol,:]+
                                                                std_factor*self.feature_dict['basestd'][parnum,imol,:])####                    
                ...
                begin_auc=exceeds_thresh_points(self.file_set_conc[par][mol],ssend_list,
                                                      self.feature_dict['auc_thresh'][parnum,imol,:],
                ...
                if np.any(np.isnan(end_auc)):
                    ...
                    self.feature_dict['auc'][parnum,imol,:]=[np.sum(self.file_set_conc[par][mol][i,self.ssend[mol]:end]-
                                                                    self.feature_dict['baseline'][parnum,imol,i])*self.dt[mol]
                                                             for i,end in enumerate(end_auc)]
       
                #calculate mean and stdev of various features
                for feat in self.feature_dict.keys():
                    self.mean_feature[feat][parnum,imol]=np.nanmean(self.feature_dict[feat][parnum,imol,:])
                    self.std_feature[feat][parnum,imol]=np.nanstd(self.feature_dict[feat][parnum,imol,:])
    ...
    def write_features(self,feature_list,arg0):    
        ...
        for feat in feature_list:
            outputdata=np.column_stack((outputdata,np.round(self.mean_feature[feat]/ms_to_sec,3),np.round(self.std_feature[feat]/ms_to_sec,3)))
        f=open(outfname, 'w')
        f.write(header+'\n')        
